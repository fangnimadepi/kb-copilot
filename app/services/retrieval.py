...
async def retrieve(query: str, db: AsyncSession) -> list[RetrievedChunk]:
    # 1) 召回：查询向量化 + Milvus ANN（pymilvus 是同步客户端，丢线程池防阻塞事件循环）
    vector = await embed_query(query)
    hits = await run_in_threadpool(vector_store.search, vector, settings.recall_top_k)
    if not hits:
        return []
    ...
    # 3) 精排 + 阈值过滤
    ranked = await rerank(query, [c.content for _, c, _ in candidates], top_n=settings.rerank_top_k)
    results = []
